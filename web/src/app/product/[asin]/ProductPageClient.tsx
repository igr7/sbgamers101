      ...
      <main className="flex-1 container-main py-10">
        ...
        <div className="grid md:grid-cols-2 gap-10 lg:gap-14">
          <div className="bg-white rounded-2xl p-8 lg:p-12 flex items-center justify-center sticky top-24 self-start">
            {product.image_url ? (
              <img src={product.image_url} alt={product.title} className="max-h-[400px] object-contain" />
            ) : (
          ...
          <div className="space-y-6">
              ...
              <div className="flex items-center gap-3">
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <svg key={i} className={`w-4 h-4 ${i < Math.round(product.rating) ? 'text-amber-400' : 'text-gray-700'}`} fill="currentColor" viewBox="0 0 20 20">
            ...
            <div className="bg-white/[0.03] rounded-2xl border border-white/[0.06] p-6 space-y-3">
              <div className="flex items-baseline gap-3 flex-wrap">
                ...
                {hasDiscount && (
                  <span className="badge-discount text-sm">-{product.discount_pct}% {t('product.off')}</span>
                )}
              </div>
              {hasDiscount && (
            ...
            <a href={product.amazon_url} target="_blank" rel="noopener noreferrer" className="btn-primary inline-flex items-center justify-center gap-2 w-full sm:w-auto text-center">
              {t('product.viewDeal')}
