...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <motion.div
      ...
      className="card-brutal overflow-hidden group h-full flex flex-col"
    >
      <Link href={`/product/${product.asin}`} className="flex flex-col h-full">
        ...
        <div className="p-3 sm:p-4 space-y-2 sm:space-y-3 bg-card flex-1 flex flex-col">
          ...
          {/* Title */}
          <h3 className="text-xs sm:text-sm font-semibold text-foreground line-clamp-2 group-hover:text-primary transition-colors leading-tight flex-1">
            {product.title}
          </h3>
